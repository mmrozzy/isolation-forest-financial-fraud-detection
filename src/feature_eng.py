...
def create_velocity_features(df):
    ...
    df['transactions_last_hour'] = ( 
        df.groupby('user_id')['amount'].rolling('1h', closed='left').count().reset_index(level=0, drop=True).fillna(0).astype(int))
    df['transactions_last_day'] = ( 
        df.groupby('user_id')['amount'].rolling('1d', closed='left').count().reset_index(level=0, drop=True).fillna(0).astype(int))
    df = df.reset_index()
    return df
...
def create_fraud_indicator_features(df):
    ...
    df['amount_last_day'] = (
        df.groupby('user_id')['amount'].rolling('1d', closed='left').sum()
        .reset_index(level=0, drop=True).fillna(0)
    )
    df = df.reset_index()
